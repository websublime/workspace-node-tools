...
pub fn js_repo_init(
    ...
    let repository = Repository::from(&PathBuf::from(cwd.clone()));
    let init = repository
...
#[napi(js_name = "repoIsUnclean", ts_return_type = "Result<boolean>")]
pub fn js_repo_is_unclean(cwd: String) -> Result<bool, RepositoryError> {
    let repository = Repository::from(&PathBuf::from(cwd.clone()));
    let is_unclean = repository.is_workdir_unclean().or_else(|_| {
        Err(Error::new(
            RepositoryError::FailRevParse,
...
pub fn js_repo_get_all_files_changed_since_branch(
    ...
    let repository = Repository::from(&PathBuf::from(cwd.clone()));
    let files = repository.get_all_files_changed_since_branch(&packages, branch.as_str());
